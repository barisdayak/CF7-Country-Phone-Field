...
const DEFAULT_COUNTRY_CODE = "+90"; // set to Turkey

// Toggle the list on/off
function toggleOptions() {
    const selectBox = document.querySelector('.select-box .options');
    ...
    const currentFieldset = document.querySelector('.cf7mls_current_fs');
    if (selectBox.classList.contains('active')) {
        currentFieldset.style.height = '400px';
    } else {
        currentFieldset.style.height = ''; 
    }
}

// Close the list when clicking on an empty field
document.addEventListener('click', function (event) {
    const selectBox = document.querySelector('.select-box .options');
    const selectedOption = document.querySelector('.selected-option div');
    const targetElement = event.target.closest('.select-box');

    if (!targetElement && selectBox.classList.contains('active')) {
        selectBox.classList.remove('active');
        selectedOption.classList.remove('active');
    }
});

// Prevent clicking on the number entry field
document.querySelector('input[type="tel"]').addEventListener('click', function (event) {
...
document.addEventListener('click', function (event) {
    ...
    if (option) {
        const selectedOption = option.closest('.select-box').querySelector('.selected-option div');
        const inputBox = option.closest('.select-box').querySelector('input[type="tel"]');
        const phoneCode = option.querySelector('strong').textContent.trim();
        const countryFlag = option.querySelector('.iconify').cloneNode(true);

        // Güncelleme işlemleri
        selectedOption.innerHTML = '';
        selectedOption.appendChild(countryFlag);
        selectedOption.insertAdjacentHTML('beforeend', ` <strong>${phoneCode}</strong>`);

        inputBox.value = `${phoneCode}`; // Only write the code in the submission field

        // Listeyi kapatma
        const selectBox = option.closest('.select-box').querySelector('.options');
        selectBox.classList.remove('active');
        selectedOption.classList.remove('active');
    }
});

// Clicker to show the selected option
document.querySelector('.selected-option div').addEventListener('click', function (event) {
    event.stopPropagation();
    toggleOptions();
});
...
document.addEventListener("DOMContentLoaded", function () {
    const selectBoxList = document.querySelectorAll(".select-box");
    selectBoxList.forEach(selectBox => {
        const optionsList = selectBox.querySelector(".options ol");
        countries.forEach(country => {
            const optionHTML = `
                <li class="option" data-code="+${country.phone}">
                    <div>
        ...
        const defaultOption = optionsList.querySelector('.option[data-code="+90"]');
        if (defaultOption) {
            const defaultFlag = defaultOption.querySelector('.iconify').cloneNode(true);
            const defaultPhoneCode = defaultOption.querySelector('strong').textContent.trim();

            const selectedOption = selectBox.querySelector('.selected-option div');
            const inputBox = selectBox.querySelector('input[type="tel"]');
            selectedOption.innerHTML = '';
            selectedOption.appendChild(defaultFlag);
            selectedOption.insertAdjacentHTML('beforeend', ` <strong>${defaultPhoneCode}</strong>`);

            // Write the default country code to input
            inputBox.value = `${defaultPhoneCode}`;
        }
    });

    // Listening for the search box
    document.body.addEventListener("input", function (event) {
        if (event.target.classList.contains("search-box")) {
            const searchBox = event.target;
            const query = searchBox.value.toLowerCase();
            const options = searchBox.closest(".select-box").querySelectorAll(".option");
